...
// Power-ups für Canvas-Zeichnung
const powerUpImages: { [key: string]: HTMLImageElement } = {
    'WEAPON_UP': createImage(powerupWeaponUpSrc),
    'RAPID_FIRE': createImage(powerupRapidFireSrc),
    'SIDE_SHOTS': createImage(powerupSideShotsSrc),
    'LASER_BEAM': createImage(powerupLaserBeamSrc),
    'HOMING_MISSILES': createImage(powerupHomingMissilesSrc),
    'SHIELD': createImage(powerupShieldSrc),
    'REPAIR_KIT': createImage(powerupRepairKitSrc),
    'EXTRA_LIFE': createImage(powerupExtraLifeSrc),
    'GHOST_PROTOCOL': createImage(powerupGhostProtocolSrc),
    'ORBITAL_DRONE': createImage(powerupOrbitalDroneSrc),
    'NUKE': createImage(powerupNukeSrc),
    'BLACK_HOLE': createImage(powerupBlackHoleSrc),
    'SCORE_BOOST': createImage(powerupScoreBoostSrc),
};

// Power-ups für HTML <img>-Tags (in der UI)
const powerUpImageSources: { [key: string]: string } = {
    'WEAPON_UP': powerupWeaponUpSrc,
    'RAPID_FIRE': powerupRapidFireSrc,
    'SIDE_SHOTS': powerupSideShotsSrc,
    'LASER_BEAM': powerupLaserBeamSrc,
    'HOMING_MISSILES': powerupHomingMissilesSrc,
    'SHIELD': powerupShieldSrc,
    'REPAIR_KIT': powerupRepairKitSrc,
    'EXTRA_LIFE': powerupExtraLifeSrc,
    'GHOST_PROTOCOL': powerupGhostProtocolSrc,
    'ORBITAL_DRONE': powerupOrbitalDroneSrc,
    'NUKE': powerupNukeSrc,
    'BLACK_HOLE': powerupBlackHoleSrc,
    'SCORE_BOOST': powerupScoreBoostSrc,
};
...
interface IUIElements {
    score: HTMLElement; level: HTMLElement; highscore: HTMLElement;
    specialInventory: HTMLElement; ultraInventory: HTMLElement; livesDisplay: HTMLElement;
    weaponStatus: HTMLElement; targetingArrow: HTMLElement; energyBar: HTMLElement;
}
...
class Entity {
    public game: Game; public pos: Vector2D; public width: number; public height: number;
    public family: string = 'none'; public type: string = 'NONE';
    protected _isGarbage: boolean = false;
    constructor(game: Game, x: number, y: number, w: number, h: number) {
        this.game = game; this.pos = new Vector2D(x, y); this.width = w; this.height = h;
    }
...
class EntityFamily extends Entity {
    constructor(game: Game, x: number, y: number, w: number, h: number, family: string, type: string) {
        super(game, x, y, w, h); this.family = family; this.type = type;
    }
}
...
class Player extends EntityFamily {
    ...
    draw(ctx: CanvasRenderingContext2D): void {
        ...
        switch (t) {
            case 1: currentImage = playerImg1; break; case 2: currentImage = playerImg2; break;
            case 3: currentImage = playerImg3; break; case 4: currentImage = playerImg4; break;
        ...
        if (this.isShielded()) {
            ...
            ctx.fillStyle = `rgba(11,255,255,${a})`; ctx.beginPath();
    ...
    takeHit(damagePercentage: number): void {
        ...
        this.powerUpManager.onPlayerHit(); this.energy -= damagePercentage;
        this.game.triggerScreenShake(5); this.game.uiManager.soundManager.play('playerHit');
        if (this.energy <= 0) {
            this.lives--;
            if (this.lives <= 0) {
                this.destroy();
                this.game.addEntity(new Explosion(this.game, this.pos.x + this.width / 2, this.pos.y + this.height / 2, '#FFFFFF', 2));
                this.game.triggerScreenShake(50); this.game.uiManager.soundManager.play('playerExplosion');
            } else {
                this.energy = 100; this.pos.x = this.game.width / 2 - this.width / 2;
                this.pos.y = this.game.height - 80; this.powerUpManager.activate('GHOST_PROTOCOL', 5000);
            }
        }
    }
...
class PowerUpManager {
    ...
    update(dt: number): void {
        for (const key in this.timers) { this.timers[key] -= dt; if (this.timers[key] <= 0) this.deactivate(key); }
        if (this.weaponTierTimer > 0) { this.weaponTierTimer -= dt; if (this.weaponTierTimer <= 0) { this.weaponTier--; this.setWeaponTierTimer(); } }
    }
    setWeaponTierTimer(): void {
        if (this.weaponTier <= 1) { this.weaponTier = 1; this.weaponTierTimer = 0; return; }
        switch(this.weaponTier) {
            case 4: this.weaponTierTimer = 30000; break; case 3: this.weaponTierTimer = 60000; break; case 2: this.weaponTierTimer = 90000; break;
        }
    }
    deactivate(key: string): void {
        if (this.ultraWeapon === key) { this.ultraWeapon = null; if (this.player.laser) { this.player.laser.destroy(); this.player.laser = null; } }
        else if (key === 'ORBITAL_DRONE') { this.player.drones.forEach(d => d.destroy()); this.player.drones = []; }
    ...
    private collectToInventory(type: string, inventory: IInventoryItem[], maxSize: number): void {
        ...
        if (existing) existing.count++; else if (inventory.length < maxSize) inventory.push({ type, count: 1 });
    ...
    activateSpecial(slotIndex: number, targetPos?: Vector2D): void {
        ...
        const special = this.specialInventory[slotIndex]; if (!special) return;
        if (special.type === 'BLACK_HOLE' && targetPos) this.game.addEntity(new BlackHole(this.game, targetPos.x, targetPos.y));
        else this.activate(special.type, 0);
        special.count--; if (special.count <= 0) this.specialInventory.splice(slotIndex, 1);
    }
    activateUltra(slotIndex: number): void {
        if (slotIndex < 0 || slotIndex >= this.ultraInventory.length) return;
        const ultra = this.ultraInventory[slotIndex]; if (!ultra || this.ultraWeapon) return;
        this.activate(ultra.type);
        ultra.count--; if (ultra.count <= 0) this.ultraInventory.splice(slotIndex, 1);
    }
    activate(type: string, duration?: number): void {
        ...
        if (type === 'WEAPON_UP') { if (this.weaponTier < 4) this.weaponTier++; this.setWeaponTierTimer(); }
            ...
            if (type === 'EXTRA_LIFE') { if (this.player.lives < this.player.maxLives) this.player.lives++; }
            else if (type === 'REPAIR_KIT') this.player.energy = 100;
            else this.timers[type] = duration ?? 15000;
        } else if (Z.includes(type)) {
            if (type === 'NUKE') { this.game.entities.filter(e => e.family === 'enemy' && !(e as Enemy).isBoss).forEach(e => (e as Enemy).takeHit(9999)); this.game.addEntity(new NukeEffect(this.game)); this.game.triggerScreenShake(50); }
            else if (type === 'SCORE_BOOST') this.timers[type] = 20000;
        }
        this.game.uiManager.soundManager.play('powerup');
    }
    shoot(): void {
...
class Projectile extends EntityFamily {
    public vel: Vector2D; public damage: number = 1;
    constructor(game: Game, x: number, y: number, velX: number = 0, velY: number = -600) {
        super(game, x - 2.5, y, 5, 20, 'projectile', 'PROJECTILE'); this.vel = new Vector2D(velX, velY);
    }
    update(dt: number): void {
    ...
    draw(ctx: CanvasRenderingContext2D): void {
        ctx.save(); ctx.fillStyle = '#0ff'; ctx.shadowColor = '#0ff'; ctx.shadowBlur = 5;
        ctx.fillRect(this.pos.x, this.pos.y, this.width, this.height); ctx.restore();
    }
...
class PowerUp extends EntityFamily {
    ...
    constructor(game: Game, x: number, y: number) {
        // Die Größe des Power-up Objekts, kann an die Größe der PNGs angepasst werden
        super(game, x, y, 32, 32, 'pickup', 'POWERUP');
    ...
    // --- ERSETZTE DRAW-METHODE ---
    draw(ctx: CanvasRenderingContext2D): void {
        // Das passende Bild aus dem globalen Objekt holen
        const image = powerUpImages[this.powerUpType];
        
        // Sicherstellen, dass das Bild existiert, bevor wir es zeichnen
        if (image) {
            ctx.save();
...
// ... (Klassen Coin, Explosion, Particle, Drone, NukeEffect, BlackHole, EnemyProjectile, SoundManager, LocalizationManager bleiben unverändert) ...
...
class UIManager {
    ...
    update(): void {
        this.scoreEl.textContent = this.game.score.toString(); this.levelEl.textContent = this.game.level > this.game.levelDefinitions.length ? 'MAX' : this.game.level.toString();
    ...
    updateWeaponStatusUI(): void {
        if (!this.game.player) return; const pm = this.game.player.powerUpManager;
        let text = `${this.localizationManager.translate('w_tier')}: ${pm.weaponTier}`;
        let timer = pm.weaponTierTimer;
        if (timer > 0 && pm.weaponTier > 1) { const seconds = Math.ceil(timer / 1000); text += ` <span class="${seconds <= 5 ? 'timer-warning' : ''}">(${seconds}s)</span>`; }
        let activeBuffs = '';
    ...
    updateTargetingArrow(): void {
        if (this.game.blackHoleTargeting.active && this.game.player) {
            this.targetingArrowEl.style.display = 'block'; const playerCenter = this.game.player.pos.y + this.game.player.height / 2;
    ...
    togglePauseMenu(isPaused: boolean): void {
        ...
        if (isPaused) { this.populateAllTranslatedContent(); this.showTab('spiel'); }
    }
    ...
    initButtons(): void {
        document.getElementById('resume-button')!.onclick = () => this.game.togglePause(); document.getElementById('restart-button')!.onclick = () => this.game.changeState('LEVEL_START', true); document.getElementById('quit-button')!.onclick = () => this.game.changeState('MENU');
        for (const key in this.tabButtons) { this.tabButtons[key]!.onclick = () => this.showTab(key); }
        const volSlider = document.getElementById('volume-master') as HTMLInputElement; volSlider.value = this.settings.masterVolume.toString();
    ...
    applySettings(): void {
        const langButton = document.getElementById('toggle-language')!; langButton.textContent = this.localizationManager.translate('lang_native_name');
    ...
    createEnemyIcon(enemyType: string): string {
        ...
        switch(enemyType) {
            case 'GRUNT': dummyEnemy = new Grunt(tempGame); break; case 'WEAVER': dummyEnemy = new Weaver(tempGame); break;
            case 'TANK': dummyEnemy = new Tank(tempGame); break; case 'SHOOTER': dummyEnemy = new Shooter(tempGame); break;
            case 'BOSS_JUGGERNAUT': dummyEnemy = new BossJuggernaut(tempGame, 1, 1); dummyEnemy.width = 30; dummyEnemy.height = 15; break;
        }
    ...
    // --- ERSETZTE populateArsenal METHODE ---
    populateArsenal(): void {
        ...
        powerupList.forEach(p => {
            ...
            if (categoryName !== currentCategory) {
                ...
                listEl.innerHTML += `<h3>- ${currentCategory} -</h3>`;
            }
            const imageSrc = powerUpImageSources[p.type];
            listEl.innerHTML += `<div class="powerup-entry">
                                   <img src="${imageSrc}" class="arsenal-icon" alt="${p.nameKey}"/>
                                   <div class="powerup-info">
                                     <div class="powerup-title">${this.localizationManager.translate(p.nameKey)}</div>
                                     <div class="powerup-desc">${this.localizationManager.translate(p.descKey)}</div>
                                   </div>
                                 </div>`;
        });
    }
    ...
    populateGegner(): void {
         const enemyList = [
            { nameKey: "gegner_grunt_name", descKey: "gegner_grunt_desc", type: 'GRUNT', strengthKey: 'strength_low' }, { nameKey: "gegner_weaver_name", descKey: "gegner_weaver_desc", type: 'WEAVER', strengthKey: 'strength_low' },
            { nameKey: "gegner_tank_name", descKey: "gegner_tank_desc", type: 'TANK', strengthKey: 'strength_medium' }, { nameKey: "gegner_shooter_name", descKey: "gegner_shooter_desc", type: 'SHOOTER', strengthKey: 'strength_medium' },
            { nameKey: "gegner_boss1_name", descKey: "gegner_boss1_desc", type: 'BOSS_JUGGERNAUT', strengthKey: 'strength_high' }, { nameKey: "gegner_boss2_name", descKey: "gegner_boss2_desc", type: 'BOSS_JUGGERNAUT', strengthKey: 'strength_high' },
            { nameKey: "gegner_boss3_name", descKey: "gegner_boss3_desc", type: 'BOSS_JUGGERNAUT', strengthKey: 'strength_extreme' }, { nameKey: "gegner_boss4_name", descKey: "gegner_boss4_desc", type: 'BOSS_JUGGERNAUT', strengthKey: 'strength_apocalyptic' },
        ];
...
class Game {
    ...
    public blackHoleTargeting: { active: boolean, slot: number, angle: number }; public stars: IStar[] = [];
    ...
    initEventListeners(): void {
        const specialKeyMap: {[key: string]: number} = {'Digit1': 0, 'Digit2': 1, 'Digit3': 2}; const ultraKeyMap: {[key: string]: number} = {'Digit4': 0, 'Digit5': 1};
        window.addEventListener('keydown', (e) => { this.keys[e.code] = true; if (this.gameState === 'PLAYING' && this.player && !this.isPaused) { if (specialKeyMap[e.code] !== undefined && !this.blackHoleTargeting.active) { const slotIndex = specialKeyMap[e.code]!; const special = this.player.powerUpManager.specialInventory[slotIndex]; if (special && special.type === 'BLACK_HOLE') { e.preventDefault(); this.blackHoleTargeting.active = true; this.blackHoleTargeting.slot = slotIndex; } } } });
        window.addEventListener('keyup', (e) => { this.keys[e.code] = false; if (this.gameState === 'PLAYING' && this.player && !this.isPaused) { if (this.blackHoleTargeting.active && specialKeyMap[e.code] === this.blackHoleTargeting.slot) { e.preventDefault(); const spawnDistance = 100; const targetPos = new Vector2D( this.player.pos.x + this.player.width / 2 + Math.cos(this.blackHoleTargeting.angle) * spawnDistance, this.player.pos.y + this.player.height / 2 + Math.sin(this.blackHoleTargeting.angle) * spawnDistance ); this.player.powerUpManager.activateSpecial(this.blackHoleTargeting.slot, targetPos); this.blackHoleTargeting.active = false; this.blackHoleTargeting.slot = -1; } } });
        window.addEventListener('keydown', (e) => { if (this.isPaused && e.code !== 'Escape') return; if (e.code === 'Enter') { e.preventDefault(); if (this.gameState === 'INTRO' || this.gameState === 'MENU') { this.uiManager.soundManager.initAudio(); this.changeState('LEVEL_START', true); } else if (this.gameState === 'GAME_OVER' || this.gameState === 'WIN') this.changeState('MENU'); } if (e.code === 'Escape') { if (this.gameState === 'PLAYING' || this.isPaused) { this.togglePause(); } } if (this.gameState === 'PLAYING' && this.player) { if (specialKeyMap[e.code] !== undefined && !this.blackHoleTargeting.active) { const special = this.player.powerUpManager.specialInventory[specialKeyMap[e.code]!]; if (special && special.type !== 'BLACK_HOLE') { this.player.powerUpManager.activateSpecial(specialKeyMap[e.code]!); } } if (ultraKeyMap[e.code] !== undefined) { this.player.powerUpManager.activateUltra(ultraKeyMap[e.code]!); } } });
    }
    ...
    changeState(newState: string, forceReset: boolean = false): void {
        if (newState === this.gameState && !forceReset) return; if (newState === 'LEVEL_START' && this.isPaused) this.togglePause(); this.gameState = newState;
        switch (newState) {
            ...
            case 'LEVEL_START': if (forceReset) { this.player = null; this.level = 0; } if (!this.player || !this.player.isAlive()) { this.level = 1; this.score = 0; this.player = new Player(this); this.entities = [this.player]; } else { this.level++; } if (this.level > this.levelDefinitions.length) { this.changeState('WIN'); return; } this.entities = this.entities.filter(e => e.family === 'player' || e instanceof Drone); this.isBossActive = false; this.scoreEarnedThisLevel = 0; this.configureLevel(); this.changeState('PLAYING_TRANSITION'); break;
            case 'PLAYING_TRANSITION': setTimeout(() => this.changeState('PLAYING'), 3000); break;
            case 'GAME_OVER': case 'WIN': this.triggerScreenShake(30); if (this.score > this.highscore) { this.highscore = this.score; localStorage.setItem('galaxyFallCelestialHighscore', this.score.toString()); } this.uiManager.soundManager.setTrack('normal'); break;
        }
    }
    configureLevel(): void {
        const levelData = this.levelDefinitions[this.level - 1]!; if (!levelData) { this.changeState('WIN'); return; }
        this.enemySpawnTypes = levelData.e; this.enemySpawnInterval = levelData.s; this.enemySpeedMultiplier = levelData.m; this.enemyHealthMultiplier = 1 + Math.floor(this.level / 5);
        this.levelMessage = this.uiManager.localizationManager.translate(levelData.msgKey); this.levelScoreToEarn = levelData.scoreToEarn;
        this.enemySpawnTimer = 0; this.uiManager.update();
        if (this.enemySpawnTypes.some(e => e.includes('BOSS'))) { this.isBossActive = true; this.spawnEnemy(); this.uiManager.soundManager.setTrack('boss'); }
    ...
    draw(): void {
        this.ctx.save(); this.ctx.clearRect(0, 0, this.width, this.height);
        if (this.shakeIntensity > 0 && this.uiManager.settings.screenShake) { const dx = (Math.random() - 0.5) * 2 * this.shakeIntensity; const dy = (Math.random() - 0.5) * 2 * this.shakeIntensity; this.ctx.translate(dx, dy); }
        this.drawParallaxStarfield(); this.entities.forEach(e => e.draw(this.ctx)); this.uiManager.drawOverlay();
