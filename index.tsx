...
const playerImg1 = createImage(playerImgSrc1);
...
interface IUIElements {
    ...
    specialInventory: HTMLElement;
    ultraInventory: HTMLElement;
    livesDisplay: HTMLElement;
    weaponStatus: HTMLElement;
    targetingArrow: HTMLElement;
...
class Entity {
    ...
    isAlive(): boolean { return !this._isGarbage; }
    destroy(): void { this._isGarbage = true; }
}

class EntityFamily extends Entity {
    constructor(game: Game, x: number, y: number, w: number, h: number, family: string, type: string) {
        super(game, x, y, w, h);
        this.family = family;
        this.type = type;
    }
...
class Player extends EntityFamily {
    ...
    public drones: Drone[] = [];
    public laser: LaserBeam | null = null;
    ...
    draw(ctx: CanvasRenderingContext2D): void {
        ...
        switch (t) {
            case 1:
                ...
                break;
            case 2:
                currentImage = playerImg2;
                break;
            case 3:
                currentImage = playerImg3;
                break;
            case 4:
        ...
        if (this.isShielded()) {
            ...
            ctx.fillStyle = `rgba(11,255,255,${a})`;
            ctx.beginPath();
            ctx.arc(this.pos.x + this.width / 2, this.pos.y + this.height / 2, this.width * 0.7, 0, Math.PI * 2);
            ctx.fill();
        }
    ...
    takeHit(damagePercentage: number): void {
        ...
        if (this.energy <= 0) {
            ...
            if (this.lives <= 0) {
                ...
                this.game.triggerScreenShake(50);
...
class PowerUpManager {
    ...
    setWeaponTierTimer(): void {
        if (this.weaponTier <= 1) {
            this.weaponTier = 1;
            this.weaponTierTimer = 0;
            return;
        }
        switch(this.weaponTier) {
            case 4: this.weaponTierTimer = 30000; break;
            case 3: this.weaponTierTimer = 60000; break;
            case 2: this.weaponTierTimer = 90000; break;
        }
    }
    ...
    collectSpecial(type: string): void {
        this.collectToInventory(type, this.specialInventory, 3);
    }
    ...
    private collectToInventory(type: string, inventory: IInventoryItem[], maxSize: number): void {
        ...
        if (existing) {
            existing.count++;
        } else if (inventory.length < maxSize) {
            inventory.push({ type, count: 1 });
        }
        this.game.uiManager.soundManager.play('powerup');
    }

    activateSpecial(slotIndex: number, targetPos?: Vector2D): void {
        if (slotIndex < 0 || slotIndex >= this.specialInventory.length) return;
        const special = this.specialInventory[slotIndex];
        if (!special) return;

        if (special.type === 'BLACK_HOLE' && targetPos) {
             this.game.addEntity(new BlackHole(this.game, targetPos.x, targetPos.y));
        } else {
    ...
    activate(type: string, duration?: number): void {
        ...
        if (type === 'WEAPON_UP') {
        ...
        } else if (type === 'ORBITAL_DRONE') {
            if (this.player.drones.length < 3) {
                this.player.drones.push(new Drone(this.game, this.player.drones.length * (Math.PI / 1.5)));
            }
            ...
            } else { // SHIELD, GHOST_PROTOCOL
                this.timers[type] = duration ?? 15000;
            }
        } else if (Z.includes(type)) {
            if (type === 'NUKE') {
    ...
    shoot(): void {
        ...
        if (this.ultraWeapon) {
            switch (this.ultraWeapon) {
                case 'LASER_BEAM':
                    ...
                    p.fireCooldown = 0; return;
                case 'HOMING_MISSILES':
                    this.game.addEntity(new HomingMissile(this.game, p.pos.x + p.width/2, p.pos.y));
                    p.fireCooldown = 400; this.game.uiManager.soundManager.play('missileLaunch'); return;
            }
        }
        ...
        switch (this.weaponTier) {
            ...
            case 2:
                this.game.addEntity(new Projectile(this.game, x + w * 0.2, y));
                this.game.addEntity(new Projectile(this.game, x + w * 0.8, y));
                break;
            case 3:
                this.game.addEntity(new Projectile(this.game, x + w / 2, y, 0, velY));
                const angle15 = 15 * (Math.PI/180);
                this.game.addEntity(new Projectile(this.game, x + w / 2, y, Math.sin(-angle15) * Math.abs(velY), Math.cos(-angle15) * velY));
                this.game.addEntity(new Projectile(this.game, x + w / 2, y, Math.sin(angle15) * Math.abs(velY), Math.cos(angle15) * velY));
                break;
            case 4:
                this.game.addEntity(new Projectile(this.game, x + w * 0.1, y, -150, velY));
                this.game.addEntity(new Projectile(this.game, x + w * 0.9, y, 150, velY));
                this.game.addEntity(new Projectile(this.game, x + w * 0.3, y));
                this.game.addEntity(new Projectile(this.game, x + w * 0.7, y));
                break;
        }
        if (this.isActive('SIDE_SHOTS')) {
            this.game.addEntity(new Projectile(this.game, x, y + h / 2, -300, 0));
            this.game.addEntity(new Projectile(this.game, x + w, y + h / 2, 300, 0));
        }
...
class LaserBeam extends EntityFamily {
    ...
    constructor(game: Game, player: Player) {
        super(game, 0, 0, 15, game.height, 'projectile', 'LASER_BEAM');
        this.player = player;
    }
    ...
    update(dt: number): void {
        ...
        this.pos.x = this.player.pos.x + this.player.width / 2 - this.width / 2;
    ...
    draw(ctx: CanvasRenderingContext2D): void {
        ctx.save();
        const x = this.pos.x, y = 0, w = this.width, h = this.height;
        const grad = ctx.createLinearGradient(x, y, x + w, y);
        grad.addColorStop(0, 'rgba(255,0,0,0)');
        grad.addColorStop(0.3, 'rgba(255,100,100,0.8)');
        grad.addColorStop(0.5, 'white');
        grad.addColorStop(0.7, 'rgba(255,100,100,0.8)');
...
class Enemy extends EntityFamily {
    ...
    public collisionDamage: number = 35;

    constructor(game: Game, x: number, y: number, w: number, h: number, health: number, points: number, type: string) {
        super(game, x, y, w, h, 'enemy', type);
        this.baseHealth = health;
        this.health = this.baseHealth * game.enemyHealthMultiplier;
    ...
    takeHit(damage: number): void {
        ...
        if (this.health <= 0 && this.isAlive()) {
            ...
            if (this.game.player && this.game.player.powerUpManager.isActive('SCORE_BOOST')) scoreToAdd *= 2;
            this.game.score += scoreToAdd;
            this.game.scoreEarnedThisLevel += scoreToAdd;
            if (this.isBoss) {
                this.game.isBossActive = false;
                this.game.changeState('LEVEL_START');
                this.game.triggerScreenShake(40);
            } else {
            ...
            if (Math.random() < 0.2) this.game.addEntity(new Coin(this.game, this.pos.x, this.pos.y, this.pointsValue));
            if (Math.random() < 0.15) this.game.addEntity(new PowerUp(this.game, this.pos.x, this.pos.y));
    ...
    update(dt: number): void {
        ...
        const dt_s = dt / 1000;
        const speedMod = this.game.player && this.game.player.powerUpManager.isActive('TIME_WARP') ? 0.3 : 1;
        this.pos.y += this.speed * speedMod * dt_s;
    ...
    drawHealthBar(ctx: CanvasRenderingContext2D): void {
        if (this.health < this.maxHealth && !this.isBoss) {
            ctx.save();
            ctx.fillStyle = '#500';
            ctx.fillRect(this.pos.x, this.pos.y - 10, this.width, 5);
            ctx.fillStyle = '#f00';
            ctx.fillRect(this.pos.x, this.pos.y - 10, this.width * (this.health / this.maxHealth), 5);
            ctx.restore();
        }
    }
}
...
class Weaver extends Enemy {
    ...
    update(dt: number): void {
        ...
        super.update(dt);
        this.angle += 3 * dt_s;
        this.pos.x += Math.sin(this.angle) * this.hSpeed * dt_s;
        ...
        if (this.attackTimer <= 0 && this.movementPattern !== 'ENTER') {
            ...
            switch (this.attackPattern) {
                case 0:
                    ...
                    break;
                case 1:
                    for (let i = 0; i < 3; i++) setTimeout(() => this.game.addEntity(new Grunt(this.game)), i * 300);
                    break;
                case 2:
                    for (let i = 0; i < 12; i++) {
...
class PowerUp extends EntityFamily {
    ...
    constructor(game: Game, x: number, y: number) {
        super(game, x, y, 25, 25, 'pickup', 'POWERUP');
        ...
        if (D.includes(this.powerUpType)) cat = 'D';

        this.color = { 'W': '#FF851B', 'U': '#B10DC9', 'D': '#00FFFF', 'Z': '#FF0000' }[cat]!;
    }
    update(dt: number): void {
    ...
    draw(ctx: CanvasRenderingContext2D): void {
        ...
        ctx.beginPath();
        ctx.arc(this.pos.x + this.width / 2, this.pos.y + this.height / 2, this.width / 1.8, 0, Math.PI * 2);
        ctx.fill();
        ctx.fillStyle = '#000';
        ctx.textAlign = 'center';
        ctx.textBaseline = 'middle';
        ctx.font = "18px 'Press Start 2P'";
        ctx.fillText(this.symbol, this.pos.x + this.width / 2, this.pos.y + this.height / 2 + 2);
        ctx.restore();
    }
    onCollect(): void {
        this.destroy();
...
class Coin extends EntityFamily {
    ...
    constructor(game: Game, x: number, y: number, value: number) {
        super(game, x, y, 15, 15, 'pickup', 'COIN');
        this.value = value;
    }
    update(dt: number): void {
        this.pos.y += this.speed * (dt / 1000);
    ...
    draw(ctx: CanvasRenderingContext2D): void {
        ...
        ctx.fillText('$', this.pos.x + this.width / 2, this.pos.y + this.height / 2 + 1);
        ctx.restore();
    }
    onCollect(): void {
        this.destroy();
...
class Particle extends Entity {
    ...
    constructor(game: Game, x: number, y: number, color: string, life: number = 0.5, size: number = 2) {
        super(game, x, y, 0, 0);
        this.family = 'effect';
        this.type = 'PARTICLE';
...
class Drone extends EntityFamily {
    ...
    constructor(game: Game, angleOffset: number) {
        super(game, 0, 0, 15, 15, 'player', 'DRONE');
...
class BlackHole extends Entity {
    ...
    update(dt: number): void {
        ...
        if (this.life <= 0) {
            this.destroy();
            this.game.addEntity(new Explosion(this.game, this.pos.x, this.pos.y, '#F0F', 50));
        }
        this.game.entities.forEach(e => {
            if (e.family === 'enemy' || e.family === 'pickup') {
                const dist = Math.hypot(this.pos.x - e.pos.x, this.pos.y - e.pos.y);
...
class SoundManager {
    ...
    defineMusicPatterns() {
        const Notes = {
            C2: 65.41, G2: 98.00, Ab2: 103.83, Eb2: 77.78,
        ...
        this.leadMelody = [
            Notes.G4, R, Notes.Eb4, R, Notes.G4, R, Notes.F4, R, Notes.Eb4, R, Notes.D4, R, Notes.C4, R, R, R,
            Notes.G4, R, Notes.Eb4, R, Notes.G4, R, Notes.F4, R, Notes.G4, Notes.Ab4, Notes.G4, Notes.F4, Notes.Eb4, R, R, R,
            Notes.Ab4, R, Notes.F4, R, Notes.Ab4, R, Notes.G4, R, Notes.F4, R, Notes.Eb4, R, Notes.C4, R, Notes.Eb4, R,
            Notes.G4, Notes.F4, Notes.Eb4, R, Notes.D4, R, Notes.C4, R, Notes.C4, R, R, R, R, R, R, R,
        ];
    ...
    defineBossMusicPatterns() {
        ...
        this.bossLeadMelody = [
            Notes.A4, Notes.A4, R, Notes.Gs4, R, Notes.A4, R, Notes.E4, Notes.F4, Notes.F4, R, Notes.E4, R, Notes.D4, R, Notes.C4,
            Notes.A4, Notes.A4, R, Notes.Gs4, R, Notes.A4, R, Notes.E4, Notes.F4, R, Notes.E4, R, Notes.D4, R, Notes.C4, R,
            Notes.A4, Notes.A4, R, Notes.Gs4, R, Notes.A4, R, Notes.E4, Notes.F4, Notes.F4, R, Notes.E4, R, Notes.D4, R, Notes.C4,
        ...
        const K = true, S = true, H = true, o = false; const repeat = (p: boolean[], t: number) => Array(t).fill(p).flat();
        this.bossKickPattern  = repeat([K,K,o,o, K,o,o,o, K,K,o,o, K,o,o,K], 4);
        this.bossSnarePattern = repeat([o,o,o,o, S,o,o,o, o,o,o,o, S,o,S,o], 4);
        this.bossHihatPattern = repeat([H,H,H,H, H,H,H,H, H,H,H,H, H,H,H,H], 4);
    }

    setTrack(trackName: 'normal' | 'boss') {
    ...
    playNote(freq: number, time: number, duration: number, type: OscillatorType, volMultiplier: number = 1) {
        ...
        const noteVol = volMultiplier * this.uiManager.settings.masterVolume;
    ...
    playDrum(type: 'kick' | 'snare' | 'hihat', time: number) {
        ...
        switch(type) {
            case 'kick': filter.type = 'lowpass'; filter.frequency.setValueAtTime(120, time); vol *= 1; duration = 0.15; break;
            case 'snare': filter.type = 'highpass'; filter.frequency.setValueAtTime(1500, time); vol *= 0.8; duration = 0.1; break;
            case 'hihat': filter.type = 'highpass'; filter.frequency.setValueAtTime(8000, time); vol *= 0.4; duration = 0.05; break;
        }
    ...
    scheduler() {
        ...
        while (this.nextNoteTime < this.audioCtx.currentTime + this.scheduleAheadTime) {
            ...
            this.playNote(arp[this.currentStep]!, this.nextNoteTime, this.stepDuration, 'square', 0.07);
            if (this.currentStep % 2 === 0) { this.playNote(bass[this.currentStep]!, this.nextNoteTime, this.stepDuration * 1.8, 'triangle', 0.3); }
            if (kick[this.currentStep]) this.playDrum('kick', this.nextNoteTime);
            if (snare[this.currentStep]) this.playDrum('snare', this.nextNoteTime);
            if (hihat[this.currentStep]) this.playDrum('hihat', this.nextNoteTime);
    ...
    toggleMusic(shouldPlay: boolean): void {
        if (!this.audioCtx) return; this.musicPlaying = shouldPlay;
        if (this.musicPlaying && this.musicScheduler === null) {
            this.currentStep = 0; this.nextNoteTime = this.audioCtx.currentTime;
            this.musicScheduler = window.setInterval(() => this.scheduler(), 25);
        } else if (!this.musicPlaying && this.musicScheduler !== null) {
            clearInterval(this.musicScheduler); this.musicScheduler = null;
        }
    }
    ...
    play(soundName: string) {
        ...
        const oscillator = this.audioCtx.createOscillator(); const gainNode = this.audioCtx.createGain();
        oscillator.connect(gainNode); gainNode.connect(this.masterGain);
        oscillator.type = type; oscillator.frequency.setValueAtTime(freq, this.audioCtx.currentTime);
        gainNode.gain.setValueAtTime(vol * this.uiManager.settings.masterVolume, this.audioCtx.currentTime);
        gainNode.gain.exponentialRampToValueAtTime(0.0001, this.audioCtx.currentTime + duration);
        oscillator.start(this.audioCtx.currentTime); oscillator.stop(this.audioCtx.currentTime + duration);
    }
}
...
class LocalizationManager {
    ...
    constructor() {
        this.setLanguage(localStorage.getItem('galaxyFallLanguage') || 'en');
    }
...
class UIManager {
    ...
    private specialInventoryEl: HTMLElement;
    private ultraInventoryEl: HTMLElement;
    private livesDisplay: HTMLElement;
    private weaponStatusEl: HTMLElement;
    private targetingArrowEl: HTMLElement;
    private energyBarEl: HTMLElement;
    private menuContainer: HTMLElement;
    private langSelectScreen: HTMLElement;
    private langBackButton: HTMLElement;
    private tabButtons: { [key: string]: HTMLButtonElement };
    private tabPanes: { [key: string]: HTMLElement };
    public settings: { masterVolume: number; music: boolean; sfx: boolean; particles: number; screenShake: boolean; };
    public soundManager: SoundManager;
    public localizationManager: LocalizationManager;
    private langSelectSource: 'startup' | 'settings' = 'startup';
    ...
    update(): void {
        ...
        this.livesDisplay.innerHTML = `<span data-translate-key="lives">${this.localizationManager.translate('lives')}</span>: <div class="life-icon"></div><span>x${this.game.player.lives}</span>`;
    ...
    updateInventoryUI(element: HTMLElement, inventory: IInventoryItem[], maxSize: number, keyStart: number): void {
        ...
        for (let i = 0; i < maxSize; i++) {
            ...
            if (item) {
                const symbol = this.localizationManager.getPowerUpSymbol(item.type);
                html += `<div class="inventory-slot"> <div class="slot-key">${i + keyStart}</div> <div class="slot-symbol">${symbol}</div> ${item.count > 1 ? `<div class="slot-count">x${item.count}</div>` : ''} </div>`;
            } else { html += `<div class="inventory-slot"><div class="slot-key">${i + keyStart}</div></div>`; }
        }
    ...
    updateWeaponStatusUI(): void {
        ...
        let text = `${this.localizationManager.translate('w_tier')}: ${pm.weaponTier}`;
        ...
        let activeBuffs = '';
        if (pm.isActive('RAPID_FIRE')) activeBuffs += ` RF(${Math.ceil(pm.timers['RAPID_FIRE']!/1000)}s)`;
        if (pm.isActive('SIDE_SHOTS')) activeBuffs += ` W&lt;&gt;(${Math.ceil(pm.timers['SIDE_SHOTS']!/1000)}s)`;
        if (pm.isActive('ORBITAL_DRONE')) activeBuffs += ` O(${this.game.player.drones.length}x) (${Math.ceil(pm.timers['ORBITAL_DRONE']!/1000)}s)`;
        this.weaponStatusEl.innerHTML = text + activeBuffs;
    }
    updateTargetingArrow(): void {
        if (this.game.blackHoleTargeting.active && this.game.player) {
            this.targetingArrowEl.style.display = 'block';
            const playerCenter = this.game.player.pos.y + this.game.player.height / 2;
            const arrowX = this.game.player.pos.x + this.game.player.width / 2;
            this.targetingArrowEl.style.top = `${playerCenter + 50}px`; this.targetingArrowEl.style.left = `${arrowX}px`;
            this.targetingArrowEl.style.transform = `translateX(-50%) translateY(-100%) rotate(${this.game.blackHoleTargeting.angle}rad)`;
        } else { this.targetingArrowEl.style.display = 'none'; }
    }
    ...
    initButtons(): void {
        document.getElementById('resume-button')!.onclick = () => this.game.togglePause();
    ...
    applySettings(): void {
        ...
        langButton.textContent = this.localizationManager.translate('lang_native_name');
        document.getElementById('toggle-music')!.textContent = this.settings.music ? this.localizationManager.translate('on') : this.localizationManager.translate('off');
        document.getElementById('toggle-sfx')!.textContent = this.settings.sfx ? this.localizationManager.translate('on') : this.localizationManager.translate('off');
        document.getElementById('toggle-particles')!.textContent = [this.localizationManager.translate('off'), this.localizationManager.translate('low'), this.localizationManager.translate('high')][this.settings.particles];
        document.getElementById('toggle-shake')!.textContent = this.settings.screenShake ? this.localizationManager.translate('on') : this.localizationManager.translate('off');
        (document.getElementById('toggle-music')! as HTMLButtonElement).classList.toggle('active', this.settings.music);
        (document.getElementById('toggle-sfx')! as HTMLButtonElement).classList.toggle('active', this.settings.sfx);
        (document.getElementById('toggle-shake')! as HTMLButtonElement).classList.toggle('active', this.settings.screenShake);
    ...
    loadSettings() {
        const saved = localStorage.getItem('galaxyFallCelestialSettings');
        return saved ? JSON.parse(saved) : { masterVolume: 0.5, music: true, sfx: true, particles: 2, screenShake: true };
    }
    populateAllTranslatedContent() {
    ...
    createEnemyIcon(enemyType: string): string {
        ...
        const tempGame = { width: 40, height: 40, enemySpeedMultiplier: 1, level: 1 } as Game;
        switch(enemyType) {
    ...
    populateGegner(): void {
         const enemyList = [
            ...
            { nameKey: "gegner_boss1_name", descKey: "gegner_boss1_desc", type: 'BOSS_JUGGERNAUT', strengthKey: 'strength_high' },
            { nameKey: "gegner_boss2_name", descKey: "gegner_boss2_desc", type: 'BOSS_JUGGERNAUT', strengthKey: 'strength_high' },
            { nameKey: "gegner_boss3_name", descKey: "gegner_boss3_desc", type: 'BOSS_JUGGERNAUT', strengthKey: 'strength_extreme' },
            { nameKey: "gegner_boss4_name", descKey: "gegner_boss4_desc", type: 'BOSS_JUGGERNAUT', strengthKey: 'strength_apocalyptic' },
        ];
        const t = (key: string) => this.localizationManager.translate(key);
        const listEl = document.getElementById('gegner-list')!; listEl.innerHTML = `<h3>- ${t('gegner_header')} -</h3>`;
        enemyList.forEach(e => {
    ...
    drawGameOver(): void {
        const ctx = this.ctx; const t = (key: string) => this.localizationManager.translate(key);
        ctx.textAlign = 'center'; ctx.fillStyle = 'rgba(0,0,0,0.7)';
        ctx.fillRect(0, 0, this.game.width, this.game.height);
        ctx.fillStyle = '#FF3333'; ctx.font = "80px 'Press Start 2P'";
        ctx.fillText(t('game_over_title'), this.game.width / 2, this.game.height / 2 - 50);
        ctx.fillStyle = '#FFF'; ctx.font = "24px 'Press Start 2P'";
        ctx.fillText(`${t('game_over_final_score')}: ${this.game.score}`, this.game.width / 2, this.game.height / 2 + 20);
        ctx.font = "20px 'Press Start 2P'";
        ctx.fillText(t('game_over_prompt'), this.game.width / 2, this.game.height / 2 + 80);
    }
    drawWinScreen(): void {
        const ctx = this.ctx; const t = (key: string) => this.localizationManager.translate(key);
        ctx.textAlign = 'center'; ctx.fillStyle = 'rgba(0,0,0,0.7)';
        ctx.fillRect(0, 0, this.game.width, this.game.height);
        ctx.fillStyle = '#39FF14'; ctx.font = "50px 'Press Start 2P'";
        ctx.fillText(t('victory_title'), this.game.width / 2, this.game.height / 2 - 50);
        ctx.fillStyle = '#FFF'; ctx.font = "24px 'Press Start 2P'";
        ctx.fillText(`${t('victory_final_score')}: ${this.game.score}`, this.game.width / 2, this.game.height / 2 + 20);
        ctx.font = "20px 'Press Start 2P'";
        ctx.fillText(t('victory_prompt'), this.game.width / 2, this.game.height / 2 + 80);
    }
    drawOverlay(): void {
        if (this.game.isBossActive) {
            const boss = this.game.entities.find(e => (e as Enemy).isBoss) as Enemy;
            if (boss) {
                const barY = 55;
                this.ctx.fillStyle = 'red';
                this.ctx.fillRect(10, barY, this.game.width - 20, 15);
                this.ctx.fillStyle = 'green';
                this.ctx.fillRect(10, barY, (this.game.width - 20) * (boss.health / boss.maxHealth), 15);
            }
        }
    }
}
...
class Game {
    ...
    public height: number;
    public keys: IKeyMap = {};
    public gameState: string = 'LANGUAGE_SELECT';
    public isPaused: boolean = false;
    private introTimer: number = 3000;
    public entities: Entity[] = [];
    public player: Player | null = null;
    public score: number = 0;
    public scoreEarnedThisLevel: number = 0;
    public level: number = 1;
    public highscore: number = 0;
    public isBossActive: boolean = false;
    public uiManager: UIManager;
    public levelDefinitions: ILevelDefinition[];
    public blackHoleTargeting: { active: boolean, slot: number, angle: number };
    public stars: IStar[] = [];

    private shakeIntensity: number = 0;
    private readonly shakeDecay: number = 0.92;

    public enemySpawnTypes: string[] = [];
    public enemySpawnInterval: number = 1200;
    private enemySpawnTimer: number = 0;
    public enemySpeedMultiplier: number = 1.0;
    public enemyHealthMultiplier: number = 1;
    public levelMessage: string = '';
    public levelScoreToEarn: number = 0;
    ...
    constructor(canvas: HTMLCanvasElement, ui: IUIElements) {
        ...
        this.levelDefinitions = [
            ...
            { scoreToEarn: 0,     s: 1000, m: 1.4, e: ['BOSS_JUGGERNAUT'], msgKey: "wave_msg_5"},
            ...
            { scoreToEarn: 0,     s: 1000, m: 1.9, e: ['BOSS_JUGGERNAUT'], msgKey: "wave_msg_10"},
            ...
            { scoreToEarn: 0,     s: 1000, m: 2.4, e: ['BOSS_JUGGERNAUT'], msgKey: "wave_msg_15"},
    ...
    initEventListeners(): void {
        const specialKeyMap: {[key: string]: number} = {'Digit1': 0, 'Digit2': 1, 'Digit3': 2};
        const ultraKeyMap: {[key: string]: number} = {'Digit4': 0, 'Digit5': 1};

        window.addEventListener('keydown', (e) => {
            this.keys[e.code] = true;
            if (this.gameState === 'PLAYING' && this.player && !this.isPaused) {
                if (specialKeyMap[e.code] !== undefined && !this.blackHoleTargeting.active) {
                    const slotIndex = specialKeyMap[e.code]!;
                    const special = this.player.powerUpManager.specialInventory[slotIndex];
                    if (special && special.type === 'BLACK_HOLE') { e.preventDefault(); this.blackHoleTargeting.active = true; this.blackHoleTargeting.slot = slotIndex; }
                }
            }
        });
        window.addEventListener('keyup', (e) => {
            ...
            if (this.gameState === 'PLAYING' && this.player && !this.isPaused) {
                if (this.blackHoleTargeting.active && specialKeyMap[e.code] === this.blackHoleTargeting.slot) {
                    e.preventDefault();
                    ...
                    this.player.powerUpManager.activateSpecial(this.blackHoleTargeting.slot, targetPos);
                    this.blackHoleTargeting.active = false; this.blackHoleTargeting.slot = -1;
                }
            }
        });
        window.addEventListener('keydown', (e) => {
             ...
             if (e.code === 'Enter') {
                ...
                else if (this.gameState === 'GAME_OVER' || this.gameState === 'WIN') this.changeState('MENU');
            }
            if (e.code === 'Escape') { if (this.gameState === 'PLAYING' || this.isPaused) { this.togglePause(); } }
            if (this.gameState === 'PLAYING' && this.player) {
                if (specialKeyMap[e.code] !== undefined && !this.blackHoleTargeting.active) {
                    const special = this.player.powerUpManager.specialInventory[specialKeyMap[e.code]!];
                    if (special && special.type !== 'BLACK_HOLE') { this.player.powerUpManager.activateSpecial(specialKeyMap[e.code]!); }
                }
                if (ultraKeyMap[e.code] !== undefined) { this.player.powerUpManager.activateUltra(ultraKeyMap[e.code]!); }
            }
        });
    }
    ...
    changeState(newState: string, forceReset: boolean = false): void {
        ...
        switch (newState) {
            ...
            case 'LEVEL_START':
                ...
                this.isBossActive = false; this.scoreEarnedThisLevel = 0;
                this.configureLevel(); this.changeState('PLAYING_TRANSITION');
                break;
            case 'PLAYING_TRANSITION': setTimeout(() => this.changeState('PLAYING'), 3000); break;
    ...
    configureLevel(): void {
        const levelData = this.levelDefinitions[this.level - 1]!;
        if (!levelData) { this.changeState('WIN'); return; }
        this.enemySpawnTypes = levelData.e; this.enemySpawnInterval = levelData.s;
        this.enemySpeedMultiplier = levelData.m; this.enemyHealthMultiplier = 1 + Math.floor(this.level / 5);
        ...
        this.enemySpawnTimer = 0; this.uiManager.update();
        if (this.enemySpawnTypes.some(e => e.includes('BOSS'))) {
            this.isBossActive = true; this.spawnEnemy(); this.uiManager.soundManager.setTrack('boss');
    ...
    update(deltaTime: number): void {
        ...
        if (this.player && !this.player.isAlive() && this.gameState === 'PLAYING') { this.changeState('GAME_OVER'); }
        this.uiManager.update();
    }
    draw(): void {
        this.ctx.save();
        this.ctx.clearRect(0, 0, this.width, this.height);
        if (this.shakeIntensity > 0 && this.uiManager.settings.screenShake) {
    ...
    createParallaxStarfield(): void {
        this.stars = [];
        for (let i = 0; i < 300; i++) {
            const l = i < 100 ? 1 : (i < 200 ? 2 : 3);
...
updateParallaxStarfield(dt: number): void {
    ...
    spawnEnemy(): void {
        ...
        switch (type) {
            ...
            case 'BOSS_JUGGERNAUT': enemy = new BossJuggernaut(this, 100 * (1 + this.level/5), 1 + this.level/10); break;
        }
        if (enemy) this.addEntity(enemy);
    }
    drawProfessionalIntro(): void {
        const t = Date.now(), w = this.width, h = this.height, ctx = this.ctx;
        ctx.textAlign = 'center'; ctx.font = "60px 'Press Start 2P'"; ctx.fillStyle = '#0ff';
        const p = Math.sin(t / 500) * 5 + 15;
        ctx.shadowColor = '#0ff'; ctx.shadowBlur = p;
        ctx.fillText("GALAXY FALL", w / 2, h / 2);
        const sG = ctx.createLinearGradient(w / 2 - 300, 0, w / 2 + 300, 0); const sP = (t % 3000) / 3000;
        sG.addColorStop(Math.max(0, sP - 0.2), 'rgba(255,255,255,0)'); sG.addColorStop(sP, 'rgba(255,255,255,0.8)'); sG.addColorStop(Math.min(1, sP + 0.2), 'rgba(255,255,255,0)');
        ctx.fillStyle = sG; ctx.fillText("CELESTIAL", w / 2, h / 2 + 60);
        ctx.shadowBlur = 0; const a = Math.sin(t / 400) * 0.4 + 0.6;
        ctx.fillStyle = `rgba(255,255,255,${a})`; ctx.font = "20px 'Press Start 2P'";
        ctx.fillText(this.uiManager.localizationManager.translate('intro_prompt'), w / 2, h / 2 + 140);
    }
    ...
    handleCollisions(): void {
        const projectiles = this.entities.filter(e => e.family === 'projectile');
        const enemies = this.entities.filter(e => e.family === 'enemy') as Enemy[];
        const player = this.player;
        if (!player || !player.isAlive()) return;

        if (player.laser && player.laser.isAlive()) {
            enemies.forEach(e => {
                if (this.isColliding(player.laser!, e)) {
                    e.takeHit(player.laser!.damage);
                    if (this.uiManager.settings.particles > 0) this.addEntity(new Particle(this, player.laser!.pos.x + player.laser!.width / 2, e.pos.y, '#FF8C00'));
                }
            });
        }
        projectiles.forEach(p => {
            if (p instanceof Projectile && p.type !== 'ENEMY_PROJECTILE') {
                enemies.forEach(e => {
                    if (p.isAlive() && e.isAlive() && this.isColliding(p, e)) { p.onHit(e); e.takeHit(p.damage); }
                });
            }
        });
        const pickups = this.entities.filter(e => e.family === 'pickup');
        pickups.forEach(p => { if (p.isAlive() && this.isColliding(player, p)) (p as PowerUp | Coin).onCollect(); });
        ...
        if (!player.isShielded() && !player.isGhosted()) {
            ...
            this.entities.filter(e => e.type === 'ENEMY_PROJECTILE').forEach(p => {
                const proj = p as EnemyProjectile;
                if (proj.isAlive() && this.isColliding(player, proj)) {
                    proj.destroy();
                    player.takeHit(proj.playerDamage);
                }
            });
        }
    }
...
window.addEventListener('load', function () {
    ...
    const uiElements: IUIElements = {
        ...
        highscore: document.getElementById('highscore')!, specialInventory: document.getElementById('special-inventory')!,
        ultraInventory: document.getElementById('ultra-inventory')!, livesDisplay: document.getElementById('lives-display')!,
